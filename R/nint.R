...
#'
#' @seealso \code{\link{nint_scatDim}}, \code{\link{nint_gridDim}}, \code{\link{nint_intvDim}}, \code{\link{nint_funcDim}}, \code{\link{nint_integrate}}, \code{\link{nint_expandSpace}}
#'
...
nint_ispace = function(x) {
    ...
    for (i in seqi(1, length(x))) {
        ...
        if (type == nint_TYPE_FUNC_DIM) {
            if (length(si) != 0) r = c(r, list(s=si))
            if (length(gi) != 0) r = c(r, list(g=gi))
            if (length(ii) != 0) r = c(r, list(i=ii))
            si = integer(0)
            gi = integer(0)
            ii = integer(0)
            fi = c(fi, i)
    ...
    if (length(gi) != 0) r = c(r, list(g=gi))
    if (length(ii) != 0) r = c(r, list(i=ii))
    if (length(fi) != 0) r = c(r, list(f=fi))
    ...
    return(mapply(function(type, i) list(i=i, g=tt[[type]](i)), names(r), r, SIMPLIFY=F))
}
...
# TODO
#nint_applyReduceInf = function(datas, x) {
    #lapply(datas, function(data) {
        #mapply(function(type, stage) {
            #if (type != 'i')
                #return(stage)
            #j = intersect(stage$i, x$i)
            #if (length(j) == 0)
                #return(stage)
            #stage$g[j,] = matrix(c(0, 1), nrow=length(j), ncol=2, byrow=T)
            #return(stage)
        #}, names(data), data, SIMPLIFY=F)
    #})
#}
...
#' @param ... other arguments passed to \code{f}.
#' @param ispaces a list of integration spaces.
#' @param transformInf transform infinite intervals to finite intervals? See \code{\link{nint_transformInf}}.
#'
#' @return \code{nint_integrate} returns a single numeric.
#'
#' @seealso \code{\link{nint_space}}, \code{\link{nint_integrateNCube}}, \code{\link{nint_integrateNFunc}}, \code{\link{nint_ispace}}, \code{\link{nint_transformInf}}
#'
#' @example examples/nint_integrate.R
#'
#' @export
nint_integrate = function(f, space=NULL, ..., ispaces=NULL, transformInf=F) {
    if (!is.null(space))
        ispaces = c(nint_ispaces(space), ispaces)
    if (length(ispaces) == 0)
        return(0)
    ...
    for (ispace in ispaces) {
        if (transformInf) {
            tt = nint_transformInf(af, ispace)
            f = tt$f
            ispace = tt$ispace
        }
